...
public final class TextShadowHelper {
    public static void drawShadowText(GuiGraphicsExtractor graphics, String text, int x, int y, int color) {
        ...
        x -= textOffset;
        // shadow
        graphics.text(font, text, (x - 1), (y - 4), 0, false);
        graphics.text(font, text, x + 1, y - 4, 0, false);
        graphics.text(font, text, x, y - 5, 0, false);
        graphics.text(font, text, x, y - 3, 0, false);

        // unrelated but it took an actual real life hour to figure out there was a drop shadow property on by default
        graphics.text(font, text, x, y - 4, color, false);
    }
}
