...
public class RegenBlocksSavedData extends SavedData {
    public record RegenEntry(BlockState targetState, long targetTick) {
        public static final Codec<RegenEntry> CODEC = RecordCodecBuilder.create(instance -> instance.group(
                BlockState.CODEC.fieldOf("state").forGetter(RegenEntry::targetState),
                Codec.LONG.fieldOf("tick").forGetter(RegenEntry::targetTick)
        ).apply(instance, RegenEntry::new));
    }

    private static final Codec<Map<BlockPos, RegenEntry>> MAP_CODEC = Codec.list(
            RecordCodecBuilder.<Pair<BlockPos, RegenEntry>>create(instance -> instance.group(
                    BlockPos.CODEC.fieldOf("pos").forGetter(Pair::getFirst),
                    RegenEntry.CODEC.fieldOf("entry").forGetter(Pair::getSecond)
    ...
    private final Map<BlockPos, RegenEntry> regenTicks;

    private RegenBlocksSavedData(Map<BlockPos, RegenEntry> regenTicks) { this.regenTicks = new HashMap<>(regenTicks); }

    public static RegenBlocksSavedData get(ServerLevel level) { return level.getDataStorage().computeIfAbsent(TYPE); }

    /**
     * @param pos block position
     * @param regenTick tick the block should
     * @param targetState the state the block gets regenerated back into
     */
    public void addBlock(BlockPos pos, long regenTick, BlockState targetState) {
        this.regenTicks.put(pos, new RegenEntry(targetState, regenTick));
        this.setDirty();
    }

    public void removeBlock(BlockPos pos) { if (this.regenTicks.remove(pos) != null) this.setDirty(); }

    public Map<BlockPos, RegenEntry> getRegenTicks() { return Collections.unmodifiableMap(regenTicks); }}